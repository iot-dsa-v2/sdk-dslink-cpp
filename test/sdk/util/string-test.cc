...
TEST(StringEncodeTest, EncodeFileName) {
  ...
  EXPECT_EQ(encode_str,
            "%21 \"#$%25&%27()%2A+,-.%2F0123456789%3A%3B%3C%3D%3E%3F@ABCDEFGHIJKLMNOPQRSTUVWXYZ[%5C]^_%60abcdefghijklmnopqrstuvwxyz{|}~%C2%A2%C3%B7");
  EXPECT_EQ(str, url_decode(encode_str));
}
TEST(StringEncodeTest, EncodeNodeName) {
  string_ encode_str = url_encode_node_name(str);
  EXPECT_EQ(encode_str,
            "%21 \"#$%25&%27()%2A+,-.%2F0123456789%3A%3B%3C%3D%3E%3F@ABCDEFGHIJKLMNOPQRSTUVWXYZ[%5C]^_%60abcdefghijklmnopqrstuvwxyz{|}~\u00a2\u00f7");
  EXPECT_EQ(str, url_decode(encode_str));
}
