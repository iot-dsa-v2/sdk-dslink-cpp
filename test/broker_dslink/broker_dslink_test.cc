...
TEST_F(BrokerDsLinkTest, Reconnect) {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    auto main_link = broker_dslink_test::create_dslink(app, port, "main-link",
                                                       false, protocol());
...
TEST_F(BrokerDsLinkTest, NotAvailableStep3) {
  ...
  auto link_1 =
      broker_dslink_test::create_dslink(app, port, "test-1", false, protocol());
  // link_1->strand->logger().level = Logger::ALL___;
  auto link_2 =
      broker_dslink_test::create_dslink(app, port, "test-2", false, protocol());
  ...
  auto unavailable_child_list = [&](const shared_ptr_<Connection> &connection,
                                    ref_<DsLinkRequester> link_req) {
    link_req->list("downstream/test-2", [&](IncomingListCache &cache,
...
TEST_F(BrokerDsLinkTest, StopTest) {
  ...
  link_2->connect([&](const shared_ptr_<Connection> connection,
    ...
    invoke_req->set_target_path("downstream/test-1/sys/stop");
